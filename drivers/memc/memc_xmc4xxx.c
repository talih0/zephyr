...
static int xmc4xxx_ebu_init(const struct device *dev)
{
	...
	for (int i = 0; i < cfg->region_len; i++) {
		...
		LOG_INF("Region %d BUSRCON 0x%x", region_index, region_config->busrcon);
		LOG_INF("Region %d BUSRAP 0x%x", region_index, region_config->busrap);
		LOG_INF("Region %d BUSWCON 0x%x", region_index, region_config->buswcon);
		LOG_INF("Region %d BUSWAP 0x%x", region_index, region_config->buswap);
